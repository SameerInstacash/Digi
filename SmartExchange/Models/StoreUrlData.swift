...
class DigiStoreUrlData: NSObject {
    
    var dictTradeIn = [String:Any]()
    var dictTradeUp = [String:Any]()
    
    init (dictTradeIn : [String:Any], dictTradeUp : [String:Any]) {
        self.dictTradeIn = dictTradeIn
        self.dictTradeUp = dictTradeUp
    }
    
    init(digiStoreUrlDict: [String: Any]) {
        
        self.dictTradeIn = digiStoreUrlDict["trade_in"] as? [String:Any] ?? [:]
        self.dictTradeUp = digiStoreUrlDict["trade_up"] as? [String:Any] ?? [:]
    
    }
    
    static func fetchDigiStoreUrlsFromFireBase(isInterNet:Bool, getController:UIViewController, completion: @escaping (DigiStoreUrlData) -> Void ) {
                
        let ref = Database.database().reference(withPath: "digi_db")
